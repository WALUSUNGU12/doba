...
const ProductImageCarousel = ({
  ...
  const prev = () => setIdx(p => (p === 0 ? image_urls.length - 1 : p - 1))
  const next = () => setIdx(p => (p === image_urls.length - 1 ? 0 : p + 1))
  ...
  const handleSwipeStart = (e: React.TouchEvent | React.MouseEvent) => {
    ...
    const onMove = (ev: TouchEvent | MouseEvent) => {
      if (!dragging) return
      const x = 'clientX' in ev ? ev.clientX : (ev as TouchEvent).touches[0].clientX
      if (Math.abs(startX - x) > 50) { dragging = false; startX - x > 0 ? next() : prev() }
    }
    const onEnd = () => {
      dragging = false
  ...
  const src = image_urls[idx].startsWith('http') ? image_urls[idx] : `https://api-doba.techgenesismw.com${image_urls[idx]}`
  ...
  return (
    <div className="relative w-full h-full group">
      <div className="w-full h-full overflow-hidden cursor-grab active:cursor-grabbing" onMouseDown={handleSwipeStart} onTouchStart={handleSwipeStart}>
        <img src={src} alt={`${product_name} ${idx + 1}`} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" draggable={false} />
      </div>
      {image_urls.length > 1 && (
        <>
          <button onClick={prev} className="absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 bg-white/80 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm" aria-label="Previous">
          ...
          <button onClick={next} className="absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 bg-white/80 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity shadow-sm" aria-label="Next">
          ...
          <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex gap-1">
            {image_urls.map((_, i) => (
              <button key={i} onClick={() => setIdx(i)} className={`h-1 rounded-full transition-all ${i === idx ? 'w-4 bg-white' : 'w-1.5 bg-white/50'}`} aria-label={`Image ${i + 1}`} />
            ))}
          </div>
          <div className="absolute top-2 right-2 bg-black/40 text-white text-xs px-2 py-0.5 rounded-full">{idx + 1}/{image_urls.length}</div>
        </>
      )}
    </div>
  )
...
export default function DashboardMarketplacePage() {
  ...
  if (isLoading) {
    return (
      <DashboardLayout role="customer" title="Marketplace">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-5">
            <div>
              <p className="text-xs font-semibold text-teal-700 uppercase tracking-widest mb-1">Mzuzu Marketplace</p>
              <h1 className="text-2xl font-bold text-gray-900">Browse Products</h1>
              <p className="text-sm text-gray-500 mt-1">Discover offerings from local merchants</p>
            </div>
            <Link
              href="/cart"
              className="inline-flex items-center gap-2 px-4 py-2.5 bg-gray-900 text-white text-sm font-semibold rounded-lg hover:bg-gray-800 transition-colors self-start sm:self-auto"
            >
        ...
        {shops.length > 0 && (
          <section className="mb-10">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-sm font-semibold text-gray-900">Verified Merchants</h2>
                  ...
                  <div className="h-20 relative overflow-hidden bg-gray-50">
                    {shop.banner_url ? (
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <div className="w-9 h-9 bg-teal-700 rounded-lg flex items-center justify-center text-white font-bold">
