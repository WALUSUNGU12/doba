...
export default function TerminalOperatorPage() {
  ...
  const [activeTab, setActiveTab] = useState<'pending' | 'out_for_delivery' | 'completed' | 'all' | 'involved_orders'>('all')
  ...
  // Order Redemption Input State
  const [orderRedeemInput, setOrderRedeemInput] = useState('')
  const [redeemNotes, setRedeemNotes] = useState('')
  const [isRedeemingByOrder, setIsRedeemingByOrder] = useState(false)

  const handleRedeemByOrder = async (action: 'collect' | 'out_for_delivery') => {
    if (!terminal) return
    if (!orderRedeemInput.trim()) {
      setMessage({ type: 'error', text: 'Please enter a valid Order ID or Order Number.' })
      return
    }
    
    setIsRedeemingByOrder(true)
    setMessage(null)
    ...
    try {
      const res = await fetch(`https://api-doba.techgenesismw.com/api/terminals/${terminal.terminal_id}/parcels/redeem-by-order`, {
        method: 'POST',
        headers,
        body: JSON.stringify({
          order_id: orderRedeemInput.trim(),
          action,
          notes: redeemNotes.trim() || `Redeemed by Order ID input`
        })
      })
      ...
      if (res.ok && data.success) {
        setMessage({ 
          type: 'success', 
          text: action === 'collect' 
            ? `Order ${orderRedeemInput} successfully REDEEMED (delivered)!`
            : `Order ${orderRedeemInput} successfully DISPATCHED (out for delivery)!`
        })
        setOrderRedeemInput('')
        setRedeemNotes('')
        loadOperatorData()
      ...
      console.error('Redeem by order error:', err)
      setMessage({ type: 'error', text: 'Server error during Order ID redemption.' })
    } finally {
      setIsRedeemingByOrder(false)
    }
  }
  ...
  return (
    <DashboardLayout role={user?.role || 'customer'} title="Terminal Operator Portal">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        {/* Quick Order Redemption Control Panel */}
        <div className="bg-white border border-gray-200 rounded-3xl p-6 shadow-sm space-y-4">
          <div className="flex items-center gap-2 text-teal-950">
            <span className="text-base">🎁</span>
            <h2 className="font-extrabold text-sm uppercase tracking-wider">Quick Order Redemption & Dispatch</h2>
          </div>
          <p className="text-xs text-gray-500 max-w-2xl leading-relaxed">
            Enter an Order ID or Order Number to redeem the parcel (mark as collected/delivered) or dispatch it for local courier delivery.
          </p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
            <div className="space-y-1">
              <label className="block text-[10px] font-black uppercase text-gray-400 tracking-wider">Order ID / Order Number</label>
              <input
                type="text"
                value={orderRedeemInput}
                onChange={(e) => setOrderRedeemInput(e.target.value)}
                placeholder="e.g. 19 or DOBA-17792..."
                className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:border-teal-600 focus:outline-none text-xs transition-all shadow-sm font-mono"
              />
            </div>
            <div className="space-y-1">
              <label className="block text-[10px] font-black uppercase text-gray-400 tracking-wider">Handover / Staff Notes (Optional)</label>
              <input
                type="text"
                value={redeemNotes}
                onChange={(e) => setRedeemNotes(e.target.value)}
                placeholder="e.g. ID verified, picked up by customer"
                className="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:border-teal-600 focus:outline-none text-xs transition-all shadow-sm"
              />
            </div>
            <div className="flex gap-2">
              <button
                type="button"
                disabled={isRedeemingByOrder || !orderRedeemInput.trim()}
                onClick={() => handleRedeemByOrder('collect')}
                className="flex-1 bg-emerald-700 hover:bg-emerald-600 disabled:bg-gray-150 disabled:text-gray-450 text-white font-extrabold text-xs uppercase tracking-wider py-3 rounded-xl transition-all shadow-sm active:scale-95 duration-100"
              >
                {isRedeemingByOrder ? 'Processing...' : '🎁 Redeem (Collected)'}
              </button>
              <button
                type="button"
                disabled={isRedeemingByOrder || !orderRedeemInput.trim()}
                onClick={() => handleRedeemByOrder('out_for_delivery')}
                className="flex-1 bg-teal-900 hover:bg-teal-850 disabled:bg-gray-150 disabled:text-gray-450 text-white font-extrabold text-xs uppercase tracking-wider py-3 rounded-xl transition-all shadow-sm active:scale-95 duration-100"
              >
                {isRedeemingByOrder ? 'Processing...' : '🚚 Dispatch (Out)'}
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-3xl overflow-hidden shadow-sm">
          ...
          {/* Control Bar */}
          <div className="p-5 border-b border-gray-255 bg-gray-50/50 flex flex-col md:flex-row md:items-center justify-between gap-4">
            {/* Filter Tabs */}
            <div className="flex flex-wrap gap-1">
              {[
                { id: 'pending', label: 'Pending Pickups', count: pendingCount },
                { id: 'out_for_delivery', label: 'Local Transit', count: outForDeliveryCount },
                { id: 'completed', label: 'Completed Pickups', count: completedCount },
                { id: 'all', label: 'All Parcels', count: totalCount },
                { id: 'involved_orders', label: 'Involved Orders', count: involvedOrders.length }
              ].map((tab) => (
                <button
                  key={tab.id}
